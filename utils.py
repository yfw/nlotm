...
def visualize(image, recon_dvae, recon_tf, attns, num_slots, N=8):
    # tile
    tiles = torch.cat(
        (
            image[:N, None, :, :, :],
    ...
    # grid
    grid = vutils.make_grid(tiles, nrow=(1 + 1 + 1 + num_slots), pad_value=0.8)
...
class LearnedPositionalEmbedding1D(nn.Module):
    def __init__(self, num_inputs, input_size, dropout=0.1):
        ...
        self.pe = nn.Parameter(
            torch.zeros(1, num_inputs, input_size), requires_grad=True
        )
